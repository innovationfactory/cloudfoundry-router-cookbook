...
# TODO Find out what these attributes do
...
default[:nginx][:path]    = "/usr/local/nginx"
default[:nginx][:vcap_log] = node[:cloudfoundry_common][:log_dir]
...
default[:lua][:path]    = "/usr/local/lua"
...
default[:lua][:plugin_source_path] = File.join(node["cloudfoundry_common"]["vcap"]["install_path"], "router", "ext", "nginx")
...
default[:nginx][:log_home] = node[:cloudfoundry_common][:log_dir]
